...
function ABdisplay($id) {
  ...
  $getxml->everything($efetch);
	...
	$ti = $getxml->atitle;
	$pd = $getxml->pdate;
	//$ab = htmlentities($getxml->abtext);
	$ab = $getxml->abtext;
	$pmid = trim($getxml->pmid);
	$pinfo = $getxml->pinfo;
	$ta = $getxml->jtitle;
  unset($getxml); 
  ...
  if ($au!="") {
    print "<b>Author</b>: $au<br>";
  }
  print "<b>Journal</b>: $ta; ";
  print "$pd; $pinfo. <b>PubMed ID</b>: $id<br>";
  if ($ab == "")
  {
	print "<b>Abstract</b>:[Not Available]";
  }
  else
  {
	 		$a = new summ();
	        ...
	        //echo str_replace(strtolower($_SESSION["long"]), strtoupper($_SESSION["abb"]), $b[$temp])." ";
	        echo $b[$temp]." ";
			  }
			echo "<p>";
			unset($a);
...
if ($from != 'da') {
  echo "<font size = \"3\"><b>PubMed for Handhelds</b> </font><br>";

	echo "<hr>";
  call_user_func('ABdisplay', $id);
	if ($from == 'tbl') {
	  echo "[<a href=/ask/asktbl.php/>New search</a>] ";
	}
}
else {
  echo "<font size = \"3\"><b>Disease Associations </b> </font><br>";
  call_user_func('ABdisplay', $id);
  echo "[<a href=da.php>Back to DA</a>] ";
}
?>
